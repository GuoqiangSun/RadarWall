...
namespace lt602 {
    ...
    // Sensor
    Sensor::Sensor()
            : mSocket(INVALID_SOCKET), mMeasureThreadHandle(THREAD_INIT_HANDLE),
              mMeasureContinuous(false), mMeasureCallback(NULL), mFrameBuffer(NULL) {
    }
    ...
    ResultCode
    Sensor::connect(const char *localIp, int localPort, const char *remoteIp, int remotePort) {
        ...
        int on = 1;
       int ret = setsockopt( mSocket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on) );
        if (ret !=0) {
            TLOGE("socket setsockopt %d", mSocket);
            mSocket = INVALID_SOCKET;
            return RC_FAILED;
        }
        ...
        res = setsockopt(mSocket, SOL_SOCKET, SO_RCVTIMEO, (const char *) &timeout,
        ...
        res = setsockopt(mSocket, SOL_SOCKET, SO_SNDTIMEO, (const char *) &timeout,
                         sizeof(struct timeval));
        if (res != 0) {
    ...
    ResultCode Sensor::setTwiceIntegrationTime(int time1, int time2) {
        ...
        uint8 command[5] = {0x30, (uint8) (time1 % 256), (uint8) (time1 / 256),
                            (uint8) (time2 % 256), (uint8) (time2 / 256)};
    ...
    ResultCode Sensor::startMeasureContinuous(MeasureCallback *callback) {
        ...
        mMeasureThreadHandle = pthread_create(&mMeasureThread, NULL, Sensor::MeasureThreadProc,
                                              this);
